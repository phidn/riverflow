...
## Why this also crystallizes the spec/story (the intent layer)

Real work is often **emergent**: the user prompts a problem, the agent does it, asks "ok?", the
user says "ok" — and a feature grows that way, never starting from a written spec. Spec-first
fights that flow, so the intent layer (`spec` + `story`) gets skipped and you can no longer answer
*"is this done? what's proven?"* without re-reading the code. The fix is **not** to force a spec up
front — it is to let the spec **crystallize at recap time** from the work that actually happened.
So a bare `rv:recap` now also writes a thin spec + stories **when the session built a feature that
has no spec/story yet** (see the emergence test in step 2b). For a one-off tiny patch, it does not.
...
| Trigger | Do |
| --- | --- |
| `rv:recap spec` | only crystallize the thin `spec` (+ its stories) for the feature this session built |
...
| `rv:recap` (no part named) / "save to riverflow" | **full package**: spec + stories (only if the session built a feature with none yet — step 2b) + ADR + worklog + wiki (wiki only if a capability was shipped this session) |

Each part is independent — keep the skill small, do exactly the asked-for slice. When the user
names `spec` or `story` explicitly, write it even for small work (they asked for it); the emergence
test only gates the **automatic** spec/story in a bare `rv:recap`.

## Artifact types

Intent layer — the "what / when is it done", crystallized from the session (see step 2b):

- **Spec (PRD)** — the thin spec of the *feature/change* this session built: summary, main flow,
  the key edge cases, acceptance criteria + a Proof table. Grained to the increment; frozen after
  ship. Acts as the **index** over the ADRs/stories the session produced.
...
The remaining riverflow types (brief/task/retro) are NOT part of this skill.
...
- **Project repo (standard riverflow):** `docs/decisions/`, `docs/worklogs/`, `docs/specs/`,
  `docs/stories/`, templates in `docs/templates/`.
- **Embedded install:** `decisions/`, `worklogs/`, `specs/`, `stories/` at the root, a
...
`worklogs/` + `wiki/` + `specs/` + `stories/`). If there is a `docs/decisions/` → standard
riverflow (specs at `docs/specs/`, stories at `docs/stories/`, wiki at `docs/wiki/`). If neither
directory exists → ask the user whether they want to initialize a decisions layer before
continuing. Find the `spec.md` / `user-story.md` / `decision.md` / `worklog.md` / `wiki.md`
templates in the repo's templates directory; if missing, take them from the riverflow repo's
`docs/templates/`.

### 2. Scan the conversation → list candidates
Re-read the current session and extract:
- **The feature/intent**: what need did this session actually serve, end-to-end? → spec/story
...
- **Verification evidence produced** (tests added/run, commands, capture/QA reports, screenshots)
  → this seeds the **Proof table** of the spec/story; note the actual links/commands.
...
### 2b. Emergence test — does this session need a spec/story?
For a **bare** `rv:recap` (full package), decide whether to auto-crystallize the intent layer.
Create a thin spec + story **only if the session built a real feature/capability**, judged by
...
is obvious) → **skip** spec/story; just do ADR/worklog/wiki as before, and say one line why.

Before creating, **check for an existing spec/story** on this topic (scan `specs/` + `stories/`
titles). If one exists → **update it** (flip status, fill the Proof table, add acceptance items)
rather than creating a duplicate. When the user explicitly typed `rv:recap spec`/`story`, skip this
...
Saving from this session (recommended):
  [1] Spec    — Live streaming pipeline (thin index) · Proof: seeded from e2e run
...
(Omit the Spec/Story lines for a one-off tiny patch — step 2b.)
...
### 5. Write the files
Write the **intent layer first** (spec → stories), then ADR → worklog → wiki, so the later
artifacts can link back to the spec/story by id.

- **Spec:** only if step 2b says so. Next sequence number = (max NNNN in the specs directory) + 1,
  four digits. Name `<NNNN>-<slug>.md`, fill the `spec.md` template. Keep it **thin** — it is an
  index over the session's stories/ADRs, not a re-plan. `Status`: `implemented` if the feature was
  built **and** verified this session, else `ready`/`draft`. Fill `Related stories` + `Related
  ADRs` with the ids you create below. Fill the **Proof** table from the evidence found in step 2
  (real links/commands); mark tiers honestly (`not yet` where there is no evidence).
- **Story:** one per finishable chunk the session delivered (often 1–3). Next number from the
  ...
  **verifiable acceptance criteria** (tick the ones actually met this session), `Lane`, `Related
  spec` → the spec above, and the **Proof** table seeded from real evidence. `Status`: `done` only
...
- **Worklog:** name `YYYY-MM-DD-<slug>.md`. Fill the frontmatter (`date/who/lane/outcome/
  ...
  spec/story/ADR ids created in this recap so the session links to its intent layer.
- **Wiki:** name `<topic-slug>.md` in the wiki dir (one topic = one page, overwritten in place).
  **Infer the topic from the session and just write it** — we can fix it later, and this is mostly
  ...
  `source_specs` to the spec id if one was written this recap. If a page for the topic already
...
- When the prompt names a part (`spec` / `story` / `wiki` / `worklog` / `adr`), do **only** that
  slice — don't recap the rest. A bare `recap` does the full package (spec/story-if-emergent + ADR
  + worklog + wiki-if-shipped).
- Spec/story is **auto-created only when the session built a real feature** (step 2b); a one-off
  tiny patch gets no spec/story. Don't write the spec first or plan ahead — crystallize from what
  was actually done. Always prefer **updating** an existing spec/story over creating a duplicate.
